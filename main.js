...
function autoFillFormFromUrl() {
    ...
    if (productName || costPrice) {
        // 寻找动态列表的第一行
        const firstRow = document.querySelector('#items-container .item-row');
        if (firstRow) {
            const modelInput = firstRow.querySelector('.item-model');
            const costInput = firstRow.querySelector('.item-cost');

            if (modelInput && productName) {
                modelInput.value = productName;
            }
            if (costInput && costPrice) {
